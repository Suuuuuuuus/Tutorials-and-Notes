...
\item Post-GWAS: 
\begin{itemize}
	\item We can use the FUMA pipeline\cite{watanabe2017functional} to carry positional, eQTL, and chromatin interaction analyses\cite{li2023ultra}:
	\begin{itemize}
		\item We can filter the imputed SNPs based on INFO score which is proportional to imputation accuracy, but too stringent INFO filter sacrifices number of SNPs.
	\end{itemize}
\end{itemize}
\end{itemize}
\textbf{lcWGS}
\begin{itemize}
\item lcWGS and GWAS\cite{rubinacci2021efficient}:
\begin{itemize}
	\item First GWAS using lcWGS (1.7×) identified two loci associated with major depressive disorder\cite{converge2015sparse}. 
	\item Another study showed that 1× WGS was able to find signals missed by standard imputation of SNP arrays\cite{gilly2016very}. 
	\item Recently, a more systematic examination of the power of GWAS based on low-coverage sequencing suggested that 1× sequencing allows discovering up to twice as many associations as standard SNP array imputation\cite{gilly2019very}.
	\item GLIMPSE used correlation, genotype discordance and non-reference discordance to measure imputation accuracy.
	\item GLIMPSE can be improved such that it does not employ any reference panel (e.g., targeted panel can have larger sample size of an existing reference panel of that population), data management, and genotype likelihood calculation.
\end{itemize}
\end{itemize}
